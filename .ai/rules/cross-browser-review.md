Read and follow this checklist after changes to `src/background/proxyChrome.js`, `src/background/proxyFirefox.js`, `src/shared/ipUtils.js`, or `src/shared/pacIpHelpers.js`. The two proxy implementations use fundamentally different APIs but must implement the same user-visible behavior. Detect asymmetries that would ship as silent regressions on one browser.

## Scope

For every review, read all four files end-to-end:
...
- For a standalone review task, use this checklist as a read-only review. Do not modify code. Do not run builds. Keep recommendations to the suggested fix sketches in the output format below.
...
- Firefox: `browser.proxy.onRequest`; HTTP authentication uses `browser.webRequest.onAuthRequired`, while SOCKS5 credentials are returned inline
...
If a standalone review finds no asymmetries, return only the "Verified Parity" section.
