...
const res = await createSubconfig(configId, { protocol: proxyProtocol })
...
```javascript
// Regular config
createSubconfig(configId, { protocol: proxyProtocol })

// Paid option config
createPaidOptionConfigSubconfig(optionId, configId, { protocol: proxyProtocol })

// Cloud server proxy
createCloudServerProxy(serverId, { protocol: proxyProtocol })
```

All return `connection_url` in format: `protocol://user:pass@host:port`.

Use the protocol selected by `getProxyProtocol()` when creating a proxy subconfig: Chrome uses HTTP, while Firefox defaults to SOCKS5 and can optionally use HTTP according to `proxy_protocol`. Do not unconditionally pass `{ protocol: "http" }` for Firefox.
