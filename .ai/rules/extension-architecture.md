...
```javascript
// Connect (listener-based; SOCKS5 auth inline, HTTP auth via onAuthRequired)
...
// Return value (HTTP routing; credentials are also supplied by onAuthRequired):
...
**Protocol selection**: User preference is stored in the `proxy_protocol` key (`"socks5"` by default on Firefox). `useProxyConnection` reads it through `getProxyProtocol()` and uses that selected protocol when creating a proxy subconfig. Firefox returns SOCKS5 credentials inline from `proxy.onRequest`; Firefox HTTP authentication uses `onAuthRequired`. Chrome proxy subconfigs and PAC behavior always use HTTP, regardless of the stored Firefox preference.
...
- The service worker has no `window`; use worker-safe globals and extension APIs
- Chrome MV3 service worker code must not use dynamic import(); the background entry must remain a single bundle
...
```text
activePage: "main" | "settings" | "splitTunnel" | "feedback"
```

Config selection and location changes expand inline inside MainPage instead of navigating to separate pages.
