...
- **Desktop:** fixed `380×600` — set on `html, body` in `popup.css`.
...
Primary screen: power toggle, connection status with external IP badge (country flag + IP), update banner (non-store installs), and an inline config selector with expiration and colored ping. The bottom card expands in place to show the config list or `LocationPicker`; changing a location disconnects and reconnects the active config when needed.
...
MainPage measures the displayed config's `locationRealId`. While its inline config list is open, it also measures every unique `locationRealId` in the list. The inline `LocationPicker` measures each available location when opened.
